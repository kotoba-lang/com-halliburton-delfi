(ns halliburton_delfi.main-test
  "Contract + behavioral test for the halliburton_delfi-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "halliburton_delfi-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'halliburton_delfi.main-test)]
