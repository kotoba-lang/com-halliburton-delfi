(ns halliburton_delfi.main
  "Kotodama WASM entrypoint for the Halliburton_delfi clean-room actor (L4) — Clojure port.
  ...
  facts (`halliburton_delfi.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "halliburton_delfi")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"halliburton_delfi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "halliburton_delfi-compat" :tier tier :entities entities} 200])
...
(defn start! [] :halliburton_delfi-compat/ready)
